...
export default function Products() {
  ...
  const CaresHead = styled.h2` font-family: 'Bebas Neue', sans-serif; font-size: 72px; line-height: 86px; color: var(--brand-primary); margin-bottom: 3.5rem; `
  const CaresGrid = styled.div` display: grid; grid-template-columns: 1fr auto 1fr; gap: 1rem; align-items: center; max-width: 1200px; margin: 0 auto; justify-items: center; `
  const CookieListWrap = styled.div` text-align: left; justify-self: end; `
  const CookieList = styled.ul` list-style: none; padding: 0; margin: 0; `
  const CookieItem = styled.li` font-family: 'Poppins', sans-serif; font-weight: 500; font-size: 22px; color: #333; margin-bottom: .7rem; white-space: nowrap; `
  const CookieItemHi = styled(CookieItem)` font-weight: 700; font-size: 32px; color: #8B4513; position: relative; margin-bottom: .9rem; &::after{ content:''; position:absolute; right:-.5rem; top:50%; width:1.75rem; height:2px; background:#8B4513; transform:translateY(-50%); } &::before{ content:''; position:absolute; right:-.8rem; top:50%; width:6px; height:6px; background:#8B4513; border-radius:50%; transform:translateY(-50%); z-index:1; } `
  const CookieImage = styled.img` width: 440px; height: 440px; object-fit: contain; `
  const IngWrap = styled.div` text-align: left; justify-self: start; `
  ...
  const IngItem = styled.li` font-size: 1.1rem; line-height: 1.5; margin-bottom: .8rem; padding-left: 1.6rem; position: relative; &::before{ content:'\2022'; position:absolute; left:0; font-weight:bold; font-size:1.3rem; } `
  const OrderBtn = styled.button` background: var(--brand-primary); color: #fff; border: none; padding: .9rem 2.2rem; border-radius: 25px; cursor: pointer; font-weight: 600; font-size: .95rem; transition: background .3s ease; text-transform: uppercase; margin-top: 1.8rem; &:hover{ background: var(--brand-accent-dark); } `
        ...
        <Grid>
          {products.map((p) => (
            <Card key={p.id}>
              <Wish />
              <ImgWrap>
                <img src="/products/img-product-default.png" alt={p.name} />
              </ImgWrap>
              <Info>
